...
function App() {
	...
	React.useEffect(() => {
		if (window.localStorage.getItem('token') || window.localStorage.getItem('google') || token || loginSuccess) {
			dispatch(getUserInfo(window.localStorage.getItem('token') || window.localStorage.getItem('google')))
		}
	}, [window.localStorage.getItem('token'), window.localStorage.getItem('google'), token, loginSuccess])
	...
	return (
		<BrowserRouter>
			{(getUserStateLoading || loading || typesLoading || eventLoading || eventsLoading) && <Loading />}
