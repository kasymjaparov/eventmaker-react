...
export default function CardEvent(props) {
    ...
    return (
        <div className='main_cards_item'>
            ...
            <div className='main_cards_item_content' style={userId && {background: 'rgb(238, 238, 238)'}}>
                <span className='main_cards_item_content_title'>{props.data.title}</span>
                <div className='main_cards_item_content_desc'>{props.data.description}</div>
                <div className='main_cards_item_content_place'>
                    <i className='fas fa-map-marker-alt main_cards_item_content_place_icon'/>
                    <span
                        className='main_cards_item_content_place_text'>{`${props.data.city} ${props.data.address}`}</span>
                </div>
                <div className='main_cards_item_content_time' style={{marginBottom: '10px'}}>
                    <Moment fromNow>{new Date(props.data.date)}</Moment>
                </div>
                {!userId &&
                <NavLink to={`/events/getUserInfo/${props.data.user}`} className='main_cards_item_content_author'>
                ...
                <div className='main_cards_item_content_href'>
                    <NavLink to={`/events/${props.data._id}`} onClick={() => dispatch(getViews())}>
                        Подробнее
                    </NavLink>
                </div>
            </div>
