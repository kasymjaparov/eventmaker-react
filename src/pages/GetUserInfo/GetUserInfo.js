      ...
      <Container>
        <div className='getuserinfo_header'>
          ...
          <div className='getuserinfo_header_info'>
            <div className='getuserinfo_header_info_username'>
              {user.username}
            </div>
            <div className='getuserinfo_header_info_email'>
              Почта: {user.email}
            </div>
            <div className='getuserinfo_header_info_email'>
              Количество постов: {user.total}
            </div>
          </div>
        </div>
        ...
        <div className='main_cards' style={{ marginBottom: "40px" }}>
          {user.events &&
            user.events
              ...
              .map(item => <CardEvent data={item} key={item._id} />)}
        </div>
        {user.events && user.events.length !== 0 && (
