...
export const getUserEvents = () => (dispatch) => {
    ...
    axios
        .get(apiGetUserEvent, {
            headers: {Authorization: `Bearer ${localStorage.getItem('token') || localStorage.getItem('google')}`}
        })
        .then(({data}) => {
...
export const postEvent = (data) => (dispatch) => {
    ...
    axios
        .post(apiPostEvent, data, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token') || localStorage.getItem('google')}`
            }
        })
        .then(({data}) => {
...
export const getTypes = () => {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch({type: constants.GET_ALL_TYPES_SUCCESS, payload: response.data.types})
        } catch (error) {
    ...
    return async (dispatch) => {
        ...
        try {
            const response = await axios.delete(`${apiDeleteEvent}/${obj._id}`, {
                headers: {Authorization: `Bearer ${localStorage.getItem('google') || localStorage.getItem('token')}`}
            })
            dispatch({type: constants.DELETE_EVENT_SUCCESS, payload: response.data.allEvents})
            dispatch({type: constants.GET_USER_EVENT_SUCCESS, payload: response.data.userEvents})
...
export const setInputValue = (value) => {
    return dispatch =>
        dispatch({type:constants.SEARCH_EVENT,payload:value})
}
